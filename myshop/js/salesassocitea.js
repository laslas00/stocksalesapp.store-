...
window.removeModalAssociate = async function(username) {
    ...
    try {
        ...
        // Get the user first to confirm they belong to the same business
        const { data: userToDelete, error: findError } = await client
            .from('users')
        ...
        if (currentBusinessId && userToDelete.business_id && userToDelete.business_id !== currentBusinessId) {
            showMessageModal('Cannot delete user from a different business.');
            return;
        }

        // Delete from users table (with business safety)
        let deleteQuery = client.from('users').delete().eq('username', username);
        ...
        showMessageModal((translations[currentLanguage]?.failedToDeleteUser || 'Failed to delete user') + ': ' + error.message);
    }
};

// Also update the removeAssociate function the same way
window.removeAssociate = async function(username) {
    ...
    try {
        ...
        if (typeof loadUsers === 'function') await loadUsers();
        if (typeof renderAssociates === 'function') renderAssociates();
        showMessageModal('User removed successfully!');
...
async function showPromotionEnvelope(promotedUsername) {
    ...
    if (inner) {
        const logoUrl = businessInfo?.logo_url || businessInfo?.logoData || '';
        if (logoUrl) {
            inner.style.backgroundImage = `url('${logoUrl}')`;
