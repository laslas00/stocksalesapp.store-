...
class UnifiedDashboard {
    ...
    // ----------------------------------------------------------------------
    async init() {
        // Ensure container is visible when reopening dashboard
        if (this.container) {
...
async fetchAllData() {
    ...
    // Helper function to fetch sales by date range (filtered by business)
    ...
    // Helper function to fetch stock (filtered by business)
    const fetchStock = async () => {
        if (!client) {
            const cached = localStorage.getItem('cached_stock');
            if (cached) {
                try {
                    const allStock = JSON.parse(cached);
                    // Filter cached stock by business
                    if (currentBusinessId) {
                        return allStock.filter(item => item.business_id === currentBusinessId || !item.business_id);
                    }
                    return allStock;
                } catch (e) {
                    return [];
        ...
        if (error) {
            console.warn('Stock fetch error:', error);
            const cached = localStorage.getItem('cached_stock');
    ...
    const [
        ...
        fetchSales(yesterday, yesterday),
        fetchSales(startOfWeek, today),
        fetchSales(lastWeekSameDay, lastWeekSameDay),
        fetchStock(),
        fetchSales(startOfMonth, endOfMonth),
        fetchSales(startOfPrevMonth, endOfPrevMonth)
    ]);
  ...
  showMessageModal(title, body, options = {}) {
    const messageModal = document.getElementById('messageModal');
        messageModalText.textContent =title, 
        messageModalBody.classList.remove('hidden');
    ...
    checkCriticalStockAlerts(stock, velocity) {
        ...
        if (alerts.length) {
            this.showMessageModal(this.t('stock_alert_title', 'Stock Alert'), alerts.join(', '));
        }
    }
    ...
    checkGoalStreak(todayTotal) {
        ...
        if (todayTotal >= goal) {
            ...
            // Celebrate
            this.showMessageModal(this.t('goal_achieved_title', 'Goal Achieved!'), 
...
renderActivityItem(sale) {
    // Get time safely from multiple possible fields
    let timeStr = 'Now';
    const timestamp = sale.timestamp || sale.created_at || sale.date_sold;
    if (timestamp) {
        try {
    ...
    return `
        <div class="activity-item">
            ...
            <div class="activity-info">
                <strong>${this.escape(sale.productName || sale.product_name || 'Unknown')}</strong>
                <span>${sale.quantity || 1} ${this.t('units_label', 'units')} • ${this.formatMoney(sale.totalAmount || sale.total_amount || 0)}</span>
            </div>
...
renderCharts() {
    ...
    if (weeklyCtx && this.state.data.daily.weekTrend) {
        ...
        const labels = [...new Set(history.map(s => s.date_sold || s.dateSold))].sort();
        const data = labels.map(date =>
            history.filter(s => (s.date_sold || s.dateSold) === date).reduce((sum, s) => sum + parseFloat(s.total_amount || s.totalAmount || 0), 0)
        );
        ...
        this.state.charts.weekly = new Chart(weeklyCtx, {
            ...
            data: { 
                labels: labels.map(l => {
                    // Safe split - handle both "YYYY-MM-DD" and other formats
                    if (!l) return '?';
                    const parts = String(l).split('-');
                    return parts.length >= 3 ? parts.slice(1).join('/') : String(l);
                }), 
                datasets: [{ 
                    label: this.t('revenue_label', 'Revenue'), 
                    data, 
                    borderColor: '#3b82f6', 
    ...
    if (monthlyCtx && this.state.data.monthly.salesData) {
        ...
        const dailyMap = {};
        salesData.forEach(s => { 
            const dateSold = s.date_sold || s.dateSold;
            if (dateSold) {
                const day = new Date(dateSold).getDate(); 
                dailyMap[day] = (dailyMap[day] || 0) + parseFloat(s.total_amount || s.totalAmount || 0); 
            }
        });
    ...
    if (stockCtx && this.state.data.stockHealth) {
        ...
        this.state.charts.stock = new Chart(stockCtx, { 
            type: 'doughnut', 
            data: { 
                labels: [this.t('healthy_label', 'Healthy'), this.t('low_stock_label', 'Low'), this.t('out_of_stock_label', 'Out')], 
                datasets: [{ 
