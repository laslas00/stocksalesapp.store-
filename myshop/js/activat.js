...
async function proceedWithActivation(code) {
    ...
    try {
        ...
        if (result.success) {
            ...
            trackAppEvent('user_mode_activated', {activeType}, null );
...
async function refreshFreeModeUI() {
    console.log('🔄 Refreshing free mode UI...');
    await checkAndHideFreeModeOnLoad();
}
...
async function enterFreeMode() {
    ...
    try {
        ...
        if (result.success) {
            ...
            localStorage.removeItem('obkyetk');
            showMessage(
                translate('freeMode.activated') || "Free Mode Activated",
                translate('freeMode.activatedDesc') || "You have 3 hours of access today. The time is tracked even if you close the app.",
                'success',
                false
            );
            localStorage.setItem('freeModeActive', 'true');
            trackAppEvent('free_mode_activated', {}, null);
            await performDeviceBackup();
            setTimeout(() => {
                sessionStorage.setItem('cameFromSplash', '1');
                window.location.href = "shop.html";
            ...
            showMessage(
                ...
                translate('freeMode.activationFailed') || "Could not activate free mode. Please try again.", 
                'error', 
                false
            );
        }
    } catch (err) {
        console.error('Free mode activation error:', err);
        showMessage(
            translate('common.error') || "Error",
            translate('freeMode.activationError') || "An error occurred. Please try again.",
            'error',
            false
        );
    }
...
function showLimitReachedMessage() {
    ...
    const existingMsg = document.getElementById('limit-msg-box');
    if (existingMsg) existingMsg.remove();
    
    // Create new limit message
    const limitMsg = document.createElement('div');
    limitMsg.id = 'limit-msg-box';
    limitMsg.className = 'business-card';
    limitMsg.style.borderColor = '#ef4444';
    limitMsg.style.background = 'linear-gradient(135deg, rgba(239, 68, 68, 0.05) 0%, rgba(239, 68, 68, 0.02) 100%)';
    limitMsg.innerHTML = `
        <div class="card-icon" style="color: #ef4444;">
            <i class="fas fa-hourglass-end"></i>
        </div>
        <h2 class="card-title" style="color: #ef4444;">${translate('freeMode.limitReached') || 'Daily Limit Reached'}</h2>
        <p class="card-description">${translate('freeMode.limitReachedDesc') || 'Your 3-hour free access for today is finished. Please activate a full license or come back tomorrow.'}</p>
        <div class="mt-4 text-sm text-gray-500 dark:text-gray-400">
            <i class="fas fa-clock mr-1"></i>${translate('freeMode.resetTime') || 'Next reset: Tomorrow at 12:00 AM'}
        </div>
    `;
...
async function checkAndHideFreeModeOnLoad() {
    ...
    try {
        ...
        if (!state.fileExists) {
            ...
            console.log('✅ Free mode available (no state file - never activated)');
            const existingMsg = document.getElementById('limit-msg-box');
            if (existingMsg) existingMsg.remove();
            return;
        }
        ...
        if (state.active === false) {
            ...
            console.log('❌ Free mode hidden (active: false - file exists but inactive)');
            showLimitReachedMessage();
            return;
        }
        if (state.limitReached === true) {
            freeCard.classList.add('hidden');
            console.log('❌ Free mode hidden (limitReached: true)');
            showLimitReachedMessage();
            return;
        }
        if (state.timeUsed >= state.dailyLimit) {
            freeCard.classList.add('hidden');
            console.log('❌ Free mode hidden (timeUsed >= dailyLimit)');
            showLimitReachedMessage();
            return;
        }
        freeCard.classList.remove('hidden');
        console.log('✅ Free mode available and active');
        const existingMsg = document.getElementById('limit-msg-box');
        if (existingMsg) existingMsg.remove();
...
if (window.electronAPI && window.electronAPI.onFreeModeLimitReached) {
    window.electronAPI.onFreeModeLimitReached(() => {
        console.log('⚠️ Free mode limit reached (received from main)');
        
        // Hide free mode card if on activate page
        if (window.location.pathname.includes('activateapp.html')) {
            const freeCard = document.getElementById('freemodecard');
        ...
        showMessage(
            translate('freeMode.limitReached') || 'Daily Limit Reached',
            translate('freeMode.limitReachedNotif') || 'Your 3-hour free access for today has ended.',
            'warning',
            false
        );
    });
