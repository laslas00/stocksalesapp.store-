...
async function loadEmailSettings() {
    try {
        ...
        if (client) {
            ...
            if (error) {
                console.warn('⚠️ Could not load email settings:', error.message);
                loadFromLocalStorage();
                return;
            }

            if (data?.email) {
                console.log("✅ Loaded email from Supabase:", data.email);
        ...
        function loadFromLocalStorage() {
            console.log("📦 Loading email from localStorage fallback");
            const savedBiz = localStorage.getItem('businessInfo');
            if (savedBiz) {
                try {
                    const biz = JSON.parse(savedBiz);
                    const emailInput = document.getElementById('alertEmail');
                    if (emailInput && biz.email) {
                        emailInput.value = biz.email;
                    }
                } catch (e) {
                    console.warn('⚠️ Could not parse localStorage businessInfo');
                }
            }
        }
...
async function saveEmailSettings() {
    ...
    try {
        ...
        if (!client) {
            showMessageModal('⚠️ Cannot connect to database. Please check your connection.');
            return;
        }

        // Check if business_info exists
        const { data: existingBiz, error: fetchError } = await client
            .from('business_info')
            .select('id')
            .limit(1)
            .maybeSingle();
        ...
        if (fetchError) {
            console.error('Error checking business info:', fetchError);
            showMessageModal('❌ Failed to save settings. Please try again.');
            return;
        }
        ...
        if (existingBiz) {
            ...
            if (updateError) {
                ...
                showMessageModal('❌ Failed to save: ' + updateError.message);
                return;
            }
        } else {
            ...
            if (insertError) {
                ...
                showMessageModal('❌ Failed to save: ' + insertError.message);
                return;
            }
        }
        ...
        // Update localStorage
        const savedBiz = localStorage.getItem('businessInfo');
        let bizInfo = savedBiz ? JSON.parse(savedBiz) : {};
        bizInfo.email = email;
        localStorage.setItem('businessInfo', JSON.stringify(bizInfo));

        // Update global businessInfo
        if (typeof businessInfo !== 'undefined') {
            businessInfo.email = email;
        }

        showMessageModal('✅ Alerts Enabled! We\'ll email ' + email);
        ...
        showMessageModal('⚠️ Something went wrong. Please try again.');
    }
}
...
function fixImagePath(path) {
    ...
    // Already absolute URL (Supabase signed URLs, external URLs)
    ...
    // Local assets - return as-is (works in browser context)
    return path;
}