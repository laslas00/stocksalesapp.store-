...
async function loadBusinessInfo2() {
  try {
    ...
    // Get business ID for targeting the right record
    ...
    if (client) {
      ...
      // Target specific business if ID is known
      if (businessId) {
      ...
      if (!error && data) {
        backendLanguage = data.language || null;
      }
    }

    // Fallback: try localStorage businessInfo
    if (!backendLanguage) {
