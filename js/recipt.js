...
async function saveReceiptToServer() {
    try {
        console.log(translate('savingReceiptToServer') || 'Saving receipt...');
        
        const client = getSB();
        if (!client) {
            showMessageModal('Database connection failed. Please try again.');
            return;
        }
        ...
        const receiptData = {
            receipt_id: document.getElementById('receiptSaleId')?.textContent || '',
            date_sold: document.getElementById('receiptDate')?.textContent || new Date().toISOString().split('T')[0],
            username: document.getElementById('receiptCashierName')?.textContent || currentUser?.username || 'Unknown',
            payment_type: document.getElementById('receiptPaymentType')?.textContent || 'Cash',
            customer_name: document.getElementById('receiptCustomerNameInput')?.value || '',
            serial_number: document.getElementById('receiptSerialNumberInput')?.value || '',
            total_amount: parseFloat(document.getElementById('receiptTotalAmount')?.textContent.replace(/[^0-9.-]+/g, '') || 0),
            subtotal: parseFloat(document.getElementById('receiptSubtotal')?.textContent.replace(/[^0-9.-]+/g, '') || 0),
            tax_amount: parseFloat(document.getElementById('receiptTax')?.textContent.replace(/[^0-9.-]+/g, '') || 0),
            advance_payment: parseFloat(document.getElementById('receiptAdvanceValueInput')?.value || '0'),
            amount_paid: parseFloat(document.getElementById('receiptAmountPaidValueInput')?.value || '0'),
            balance_due: parseFloat(document.getElementById('receiptBalanceValueInput')?.value || '0'),
        ...
        if (!receiptData.receipt_id) {
            showMessageModal(translate('receiptIdRequired') || 'Receipt ID is required');
            return;
        }
        
        if (items.length === 0) {
            showMessageModal(translate('atLeastOneItemRequired') || 'At least one item is required');
            return;
        }
        
        if (typeof showLoading === 'function') showLoading();
        
        // Save each item as a separate sale record WITH business_id
        const salesToInsert = items.map(item => ({
            ...
            date_sold: receiptData.date_sold,
            username: receiptData.username,
            receipt_id: receiptData.receipt_id,
            serial_number: item.serial_number || receiptData.serial_number,
            tax_amount: receiptData.tax_amount / (items.length || 1),
            advance_payment: receiptData.advance_payment / (items.length || 1),
        ...
        showMessageModal(translate('receiptSavedSuccess') || 'Receipt saved successfully!', 'success');
        
        if (typeof closeAllEditableRows === 'function') closeAllEditableRows();
        
        const saveBtn = document.getElementById('saveReceiptModificationsBtn');
        if (saveBtn) saveBtn.classList.add('hidden');
        
        console.log(`✅ Receipt saved: ${salesToInsert.length} items (business: ${currentBusinessId || 'all'})`);
        
    } catch (error) {
        if (typeof hideLoading === 'function') hideLoading();
        console.error('Error saving receipt:', error);
        showMessageModal(
            (translate('failedToSaveReceiptPrefix') || 'Failed to save receipt: ') + error.message
        );
    }
}
...
async function printReceipt() {
    ...
    try {
        ...
        trackAppEvent('receipt_printed', {}, null);
...
document.getElementById('saveReceiptModificationsBtn')?.addEventListener('click', async function () {
    ...
    try {
        ...
        // Check if receipt exists (with business filter)
        let checkQuery = client.from('sales').select('id').eq('receipt_id', saleId);
        if (currentBusinessId) checkQuery = checkQuery.eq('business_id', currentBusinessId);
        
        const { data: existingSale, error: checkError } = await checkQuery.limit(1).maybeSingle();

        if (checkError) throw checkError;

        if (existingSale) {
            // Update existing receipt (with business safety)
            let updateQuery = client.from('sales').update({
                advance_payment: parseFloat(advanceText) || 0,
                amount_paid: parseFloat(paidText) || 0,
                balance_due: parseFloat(balanceText) || 0,
                updated_at: new Date().toISOString()
            }).eq('receipt_id', saleId);
            if (currentBusinessId) updateQuery = updateQuery.eq('business_id', currentBusinessId);

            const { error: updateError } = await updateQuery;
            if (updateError) throw updateError;
            
            console.log('✅ Receipt updated:', saleId);
        } else {
            // Create new receipt
            const items = currentCustomReceiptData?.items || [];
            const date = document.getElementById('receiptDate')?.textContent.trim() || new Date().toISOString().split('T')[0];
            const customerName = getMostFrequentCustomerName(items) || currentCustomReceiptData?.customer || 'Unknown';
            ...
            const salesToInsert = items.map(item => ({
                ...
                date_sold: date,
                receipt_id: saleId,
                advance_payment: parseFloat(advanceText) / (items.length || 1) || 0,
                amount_paid: parseFloat(paidText) / (items.length || 1) || 0,
                balance_due: parseFloat(balanceText) / (items.length || 1) || 0,
                business_id: currentBusinessId,
                username: currentUser?.username || 'Unknown',
                created_at: new Date().toISOString()
            }));

            const { error: insertError } = await client.from('sales').insert(salesToInsert);
            if (insertError) throw insertError;
            
            console.log('✅ New receipt created:', saleId, 'with', salesToInsert.length, 'items');
        }
    ...
    try {
        ...
        if (String(receiptId).startsWith('CR-')) {
            ...
            if (!salesError && salesData && salesData.length > 0) {
                const receiptData = {
                    ...
                    paymentType: salesData[0].payment_type,
                    cashier: salesData[0].username,
                    items: salesData.map(sale => ({
                        id: sale.id, name: sale.product_name, type: sale.type || 'product',
                        quantity: sale.quantity || 1, price: sale.total_amount || sale.price || 0,
        ...
        if (!singleError && singleSale) {
            const receiptData = {
                ...
                items: [{ id: singleSale.id, name: singleSale.product_name, type: singleSale.type || 'product', quantity: singleSale.quantity || 1, price: singleSale.total_amount || singleSale.price || 0, paymentType: singleSale.payment_type || 'Cash', customerName: singleSale.customer_name }],
                total: singleSale.total_amount || singleSale.price || 0,
...
if (businessLogo) {
    ...
    businessLogo.classList.toggle('hidden', !logoUrl);
}
...
async function loadCustomerReceipts() {
    try {
        ...
        if (client) {
            ...
            // Get from Supabase sales table (filtered by business)
            let query = client
                .from('sales')
                .select('customer_name, id, product_name, date_sold')
        ...
        // Fallback: try localStorage if Supabase returned nothing
        if (receipts.length === 0) {
            const savedSales = localStorage.getItem('cached_sales');
            if (savedSales) {
                try {
                    ...
                    receipts = sales
                        .filter(s => {
                            if (currentBusinessId && s.business_id && s.business_id !== currentBusinessId) return false;
                            return s.customerName || s.customer_name;
                        })
                        .map(s => ({
                            customerName: s.customerName || s.customer_name,
                            receiptId: s.id || s.receiptId,
                            productName: s.productName || s.product_name,
                            date: s.dateSold || s.date_sold || s.date
                        }));
                } catch (e) {
                    console.warn('Could not parse cached sales for receipts');
                }
            }
        }
        ...
        console.log(`✅ Loaded ${receipts.length} receipts, ${customerNames.size} unique customers`);
...
// Search and display receipt by ID
document.getElementById('searchCustomerReceiptBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (typeof hideLoading === 'function') hideLoading();
        showMessageModal(
            (translate('errorLoadingReceipt') || 'Error loading receipt') + ': ' + error.message
        );
    }
});
